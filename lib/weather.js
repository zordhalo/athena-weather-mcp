...
/**
 * Active NWS alerts affecting a point (US only).
 *
 * `?point=` alone is too strict: it only matches alerts carrying a polygon that
 * literally contains the coordinate, so county-wide warnings are missed. NWS
 * models alerts against *zones*, so we resolve the point to its county and
 * forecast zone via /points and query those, then union with the point query.
 */
export async function fetchAlertsForPoint(lat, lon) {
  ...
  const zoneIds = await getJSON(`https://api.weather.gov/points/${pt}`, geo)
    .then((p) => [p?.properties?.county, p?.properties?.forecastZone]
      .filter(Boolean)
      .map((u) => u.split('/').pop()))
    .catch(() => []);

  const queries = [
    getJSON(`https://api.weather.gov/alerts/active?point=${pt}`, geo).catch(() => null),
    ...zoneIds.map((z) =>
      getJSON(`https://api.weather.gov/alerts/active?zone=${z}`, geo).catch(() => null)
    ),
  ];
...
export async function fetchAlerts({ area, severity } = {}) {
  // NB: api.weather.gov rejects a `limit` param on /alerts/active with a 400,
  // so the result set is trimmed by the caller instead.
  const params = new URLSearchParams();
  if (area) params.set('area', area.toUpperCase());
  if (severity) params.set('severity', severity);
  const qs = params.toString();
  const data = await getJSON(
    `https://api.weather.gov/alerts/active${qs ? `?${qs}` : ''}`,
...
export async function buildExplorerPayload(locationQuery, days = 7) {
  ...
  // Every 2nd hour: that is what the chart plots, and halving the series keeps
  // the payload small enough to travel as structuredContent to the widget.
