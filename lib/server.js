...
import { renderWidget, renderWidgetTemplate } from './widget.js';
 ...
 */
/**
 * Variant flags, used to A/B the render path on preview deployments without
 * touching production. See api/index.js for the path -> variant mapping.
 *
 *   mimeProfile : advertise text/html;profile=mcp-app (Anthropic apps surface)
 *   templates   : advertise the parameterised ui:// resource as a template
 *   stripUiKey  : omit _meta.ui.resourceUri, forcing the inline embedded path
 */
export const VARIANTS = {
  base:     { mimeProfile: false, templates: false, stripUiKey: false },
  mime:     { mimeProfile: true,  templates: false, stripUiKey: false },
  template: { mimeProfile: false, templates: true,  stripUiKey: false },
  nokey:    { mimeProfile: false, templates: false, stripUiKey: true },
  all:      { mimeProfile: true,  templates: true,  stripUiKey: false },
};

const APP_MIME = 'text/html;profile=mcp-app';
const htmlMime = (v) => (v.mimeProfile ? APP_MIME : 'text/html');

function toolResult({ html, summary, structured, uri = WIDGET_URI }, v = VARIANTS.base) {
  const meta = {
    'openai/outputTemplate': uri,
    'mcpui.dev/ui-preferred-frame-size': ['100%', '760px'],
  };
  // The known key: hosts that recognise it resolve the URI via resources/read
  // instead of using the inline embedded resource below.
  if (!v.stripUiKey) meta.ui = { resourceUri: uri };
...
async function callTool(name, args = {}, v = VARIANTS.base) {
  if (name === 'explore_weather') {
    ...
    return toolResult({
      ...
      // The full payload travels as structuredContent because that is the
      // channel template-fetching hosts hand to the widget via ontoolresult.
      // A trimmed summary here would render a widget that silently disagrees
      // with the text answer beside it.
      structured: payload,
    }, v);
  }
...
export async function handleRpc(msg, v = VARIANTS.base) {
  ...
  switch (method) {
    ...
    case 'tools/call':
      try {
        return ok(await callTool(params.name, params.arguments ?? {}, v));
      } catch (err) {
    ...
    case 'resources/templates/list':
      // The per-location URI is parameterised, so it can only be advertised as
      // a template. Hosts that validate _meta.ui.resourceUri against declared
      // resources need this to resolve ui://...?location=... at all.
      return ok({
        resourceTemplates: v.templates
          ? [{
              uriTemplate: `${WIDGET_URI}{?location,days}`,
              name: 'Weather Explorer Widget (by location)',
              description: 'Interactive weather explorer for a specific location.',
              mimeType: htmlMime(v),
            }]
          : [],
      });
    ...
    case 'resources/read': {
      ...
      // No location in the URI means the host resolved the tool's *static*
      // resourceUri — it intends to fetch this once as a template and push the
      // tool result in afterwards. Serve the self-hydrating build.
      const html = payload ? renderWidget(payload) : renderWidgetTemplate();
