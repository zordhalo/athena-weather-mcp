 ...
 */
function toolResult({ html, summary, structured }) {
  return {
    content: [
...
async function callTool(name, args = {}) {
  ...
  if (name === 'list_active_alerts') {
    ...
    const alerts = sortAlerts(
      await fetchAlerts({ area: args.area, severity: args.severity, limit: 200 })
    ).slice(0, limit);
...
export async function handleRpc(msg) {
  ...
  switch (method) {
    ...
    case 'resources/read': {
      // Served with a neutral sample so a host that pre-fetches the template
      // before any tool call still renders something coherent.
      const payload = await buildExplorerPayload('New York, New York', 7).catch(() => null);
      const html = payload
        ? renderWidget(payload)
        : '<!doctype html><meta charset="utf-8"><p style="font:14px system-ui">Call <b>explore_weather</b> to load a location.</p>';
      return ok({
        contents: [{
          uri: params.uri ?? WIDGET_URI,
          mimeType: 'text/html',
          text: html,
          _meta: { 'mcpui.dev/ui-preferred-frame-size': ['100%', '760px'] },
        }],
