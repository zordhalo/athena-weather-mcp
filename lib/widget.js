/**
 * The interactive weather widget, in two shapes from one implementation.
 *
 *   renderWidget(payload)   — data inlined at render time. Zero network, zero
 *                             host cooperation. Used by hosts that render the
 *                             embedded resource from the tool result (Athena)
 *                             and by the standalone /widget preview route.
 *
 *   renderWidgetTemplate()  — no data. Fetched ONCE as a static template, then
 *                             hydrated at runtime from the tool result. This is
 *                             the model Claude's apps surface uses: it resolves
 *                             the tool's static _meta.ui.resourceUri and pushes
 *                             data in via the ext-apps bridge afterwards.
 *
 * Both share the same boot() renderer below, so the UI can never drift between
 * the two paths.
 */

import EXT_APPS_BUNDLE from './extAppsBundle.js';
...
export function renderWidget(payload) {
  const title = [payload.place.name, payload.place.region, payload.place.country]
    .filter(Boolean)
    .join(', ');
  return html({ title: `Weather Explorer — ${esc(title)}`, data: safeJSON(payload), bundle: '' });
}
...
export function renderWidgetTemplate() {
  return html({
    ...
    data: 'null',
    bundle: EXT_APPS_BUNDLE,
  });
}

function html({ title, data, bundle }) {
  return `<!doctype html>
<html lang="en">
<meta charset="utf-8" />
...
<style>
  :root {
    ...
    --ink: #10151c; --ink-dim: #5c6673; --line: #dfe3e9;
  ...
  * { box-sizing: border-box; }
  ...
  .day { min-width: 84px; background: var(--panel); border: 1px solid var(--line);
  ...
  .day[aria-selected="true"] { border-color: var(--accent); background: var(--accent-soft);
  ...
  .metric { border: 0; background: transparent; color: var(--ink-dim); cursor: pointer;
  ...
  .chart-card { background: var(--panel); border: 1px solid var(--line);
                border-radius: var(--radius); padding: 12px 8px 6px; margin-top: 8px;
                overflow-x: auto; }
  svg { display: block; }
  ...
  .alert { border: 1px solid var(--line); border-left: 4px solid var(--sev);
           background: var(--panel); border-radius: 10px; margin-bottom: 8px; overflow: hidden; }
  .alert-top { display: flex; gap: 10px; align-items: center; padding: 10px 12px;
               cursor: pointer; user-select: none; }
  .alert-top:hover { background: var(--panel-2); }
  .sev-tag { color: var(--sev); border: 1px solid var(--sev); border-radius: 5px;
  ...
  #waiting { padding: 28px 16px; text-align: center; color: var(--ink-dim); font-size: 13px; }
  #app[hidden] { display: none; }
</style>

<div id="waiting">🌤️ Loading weather explorer…</div>
...
(function () {
  ...
  var INLINE_DATA = ${data};
  var booted = false;
  var host = null;

  /* ---------------- host bridge ---------------- */

  // Pull the widget payload out of whatever shape the host hands us.
  function extract(result) {
    if (!result) return null;
    if (result.place && result.daily) return result;                 // payload itself
    var sc = result.structuredContent;
    if (sc && sc.place && sc.daily) return sc;                       // MCP structuredContent
    if (sc && sc.widget && sc.widget.place) return sc.widget;
    var content = result.content;
    if (Array.isArray(content)) {                                    // JSON in a text block
      for (var i = 0; i < content.length; i++) {
        var c = content[i];
        if (c && c.type === 'text' && typeof c.text === 'string' && c.text.charAt(0) === '{') {
          try {
            var parsed = JSON.parse(c.text);
            if (parsed.place && parsed.daily) return parsed;
  ...
  function connectHost() {
    // 1. ext-apps bridge (Anthropic apps surface)
    try {
      var ExtApps = globalThis.ExtApps;
      if (ExtApps && ExtApps.App) {
        host = new ExtApps.App(
          { name: 'WeatherExplorer', version: '1.0.0' }, {}, { autoResize: true }
        );
        host.ontoolresult = receive;
        host.onhostcontextchanged = applyTheme;
        host.connect().then(function () {
          try { applyTheme(host.getHostContext && host.getHostContext()); } catch (e) {}
        }).catch(function () {});
      }
    } catch (e) { /* bridge unavailable — other channels still apply */ }

    // 2. OpenAI Apps SDK globals
    try {
    ...
    // 3. Raw postMessage fallback
    window.addEventListener('message', function (e) {
      var d = e && e.data;
      if (!d) return;
      receive(d.result || d.toolOutput || d.payload || d);
    });
  }

  /* ------------- outbound: tell the host what the user did ------------- */
  function notifyHost(text) {
    var msg = { role: 'user', content: [{ type: 'text', text: text }] };
    try { if (host && host.sendMessage) { host.sendMessage(msg); return; } } catch (e) {}
  ...
  function boot(DATA) {
    if (booted) return;          // first payload wins; re-render guarded
    booted = true;
    document.getElementById('waiting').style.display = 'none';
    document.getElementById('app').hidden = false;

    var state = { dayIndex: 0, metric: 'temp' };
    var METRICS = {
    ...
    [
      ...
      DATA.alerts.length
        ...
        : 'No active alerts'
    ].forEach(function (t) { chipsEl.append(mk('span', 'chip', t)); });
    ...
    DATA.daily.forEach(function (d, i) {
      ...
      var t = mk('div');
      t.append(mk('span', 'hi', d.tempMax + '°'), document.createTextNode(' '),
               mk('span', 'lo', d.tempMin + '°'));
      b.append(t);
      b.append(mk('div', 'pp', d.precipChance > 0 ? '💧 ' + d.precipChance + '%' : ''));
      var pick = function () { selectDay(i); };
    ...
    function selectDay(i) {
      ...
      for (var n = 0; n < kids.length; n++) kids[n].setAttribute('aria-selected', String(n === i));
      draw();
      var d = DATA.daily[i];
      notifyHost(
    ...
    var metricBtns = document.querySelectorAll('.metric');
    Array.prototype.forEach.call(metricBtns, function (btn) {
    ...
    function baseReadout(day) {
      document.getElementById('readout').textContent =
        ...
        ' · 💨 Max wind ' + day.windMax + ' mph (gusts ' + day.gustMax + ') · 🌧️ Total ' +
        day.precipSum + ' in';
    }
    ...
    function wire(node, h, day) {
      ...
      node.addEventListener('mouseleave', function () { baseReadout(day); });
    }

    function draw() {
      var day = DATA.daily[state.dayIndex];
      var hours = DATA.hourlyByDate[day.date] || [];
      ...
      var W = Math.max(hours.length * 58, 640), H = 180;
      var padL = 12, padR = 12, padT = 26, padB = 26;
      svg.setAttribute('width', W);
      svg.setAttribute('viewBox', '0 0 ' + W + ' ' + H);

      var vals = hours.map(function (h) { return h[m.key]; });
      var lo = Math.min.apply(null, vals), hi = Math.max.apply(null, vals);
      if (hi === lo) hi = lo + 1;
      var pad = (hi - lo) * 0.18;
      var yMin = m.max ? 0 : lo - pad, yMax = m.max ? m.max : hi + pad;
      var plotH = H - padT - padB;
      var x = function (i) { return padL + (i + 0.5) * ((W - padL - padR) / hours.length); };
      var y = function (v) { return padT + plotH - ((v - yMin) / (yMax - yMin)) * plotH; };

      for (var g = 0; g <= 2; g++) {
        var yy = padT + (plotH / 2) * g;
        svg.append(sel('line', { x1: padL, x2: W - padR, y1: yy, y2: yy, class: 'gridline' }));
      }

      if (m.mode === 'bar') {
        var bw = Math.min(30, (W - padL - padR) / hours.length - 10);
        hours.forEach(function (h, i) {
          var yv = y(h[m.key]);
          var r = sel('rect', {
            x: x(i) - bw / 2, y: yv, width: bw,
            height: Math.max(padT + plotH - yv, 1), rx: 4, class: 'hbar'
          });
          wire(r, h, day);
          svg.append(r);
        });
      } else {
        var dPath = hours.map(function (h, i) {
          return (i ? 'L' : 'M') + x(i) + ' ' + y(h[m.key]);
        }).join(' ');
        svg.append(sel('path', { d: dPath, class: 'hline' }));
        hours.forEach(function (h, i) {
    ...
    if (!DATA.alerts.length) {
      ...
      var note = mk('span', null, DATA.alertCoverage);
  ...
  if (INLINE_DATA) {
    ...
    connectHost();
    // If no host ever speaks up, say so rather than spinning forever.
    setTimeout(function () {
      if (!booted) {
